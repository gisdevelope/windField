...
var Vector = function Vector(c, d) {
  ...
  var d = this.length();if (d) {
    var e = c / d;this.x *= e, this.y *= e;
  }return this;
}, Vector.prototype.setAngle = function (c) {
  var d = length();return this.x = d * Math.cos(c), this.y = d * Math.sin(c), this;
}, Vector.prototype.getAngle = function () {
  return Math.atan2(this.y, this.x);
}, Vector.prototype.d = function (c) {
  var d = c.x - this.x,
      e = c.y - this.y;return Math.sqrt(d * d + e * e);
};var VectorField = function VectorField(c, d, e, f, k) {
  this.x0 = d, this.x1 = f, this.y0 = e, this.y1 = k, this.field = c, this.w = c.length, this.h = c[0].length, this.maxLength = 0;for (var l = 0, o = 0, q = 0; q < this.w; q++) {
  ...
  for (var e = [], f = c.gridWidth, k = c.gridHeight, o = 0, q = 0, s = 0, t = 0; t < f; t++) {
    e[t] = [];for (var u = 0; u < k; u++) {
      var z = c.field[o++],
          A = c.field[o++],
          B = new Vector(z, A);if (d) {
        var C = t / (f - 1),
            D = u / (k - 1),
            E = c.x0 * (1 - C) + c.x1 * C,
            F = c.y0 * (1 - D) + c.y1 * D,
            G = Math.PI * F / 180,
            H = B.length();H && (q += H * G, s += G), B.x /= Math.cos(G), B.setLength(H);
      }e[t][u] = B;
    }
  }var I = new VectorField(e, c.x0, c.y0, c.x1, c.y1);return q && s && (I.averageLength = q / s), I;
  ...
  var f = Math.floor(d),
      ...
      o = Math.ceil(e),
      q = d - f,
      s = e - k;return this.field[f][k][c] * (1 - q) * (1 - s) + this.field[l][k][c] * q * (1 - s) + this.field[f][o][c] * (1 - q) * s + this.field[l][o][c] * q * s;
  ...
  var f = (this.w - 1 - 1e-6) * (c - this.x0) / (this.x1 - this.x0),
      ...
      o = this.bilinear('y', f, k);return e ? (e.x = l, e.y = o, e) : new Vector(l, o);
}, VectorField.prototype.vectValue = function (c) {
  return this.getValue(c.x, c.y);
}, VectorField.constant = function (c, d, e, f, k, l) {
  var o = new VectorField([[]], e, f, k, l);return o.maxLength = Math.sqrt(c * c + d * d), o.getValue = function () {
  ...
  if (!this.stop) for (var e, d = 0; d < this.listeners.length; d++) {
    e = this.listeners[d], e[c] && e[c].call(e, this);
  }
}, Animator.prototype.start = function (c) {
  function d() {
    var k = new Date();f.notify('animate');var l = new Date() - k;setTimeout(d, Math.max(10, (c || 20) - l));
  }var f = this;d();
  ...
  this.canvas = c, this.projection = function (f, k, l) {
    var o = l || new Vector(),
        q = e.latLngToContainerPoint([k, f]);if (0 > q.x || q.x > c.width) {
      var s = e.latLngToContainerPoint([0, 180]),
          t = e.latLngToContainerPoint([0, -180]),
          u = -t.x / 10;if (s.x < c.width + u) {
        var z = c.width - s.x;q.x < t.x + z + u && (q.x = s.x + (q.x - t.x));
      }var A = (s.x - c.width) / 10;t.x > 0 - A && q.x > s.x - t.x - A && (q.x = t.x - (s.x - q.x));
  ...
  this.particles = [];for (var d = 0; d < this.numParticles; d++) {
  ...
  for (var d = c ? c.dx : 0, e = c ? c.dy : 0, f = c ? c.scale : 1, k = 0;;) {
    var l = Math.random(),
        ...
        t = this.field.getValue(q, s);if (0 == this.field.maxLength) return new Particle(q, s, 1 + 40 * Math.random());var u = t.length() / this.field.maxLength;if ((t.x || t.y) && (10 < ++k || Math.random() > .9 * u)) {
      var z = this.projection(q, s),
          A = z.x * f + d,
          B = z.y * f + e;if (10 < ++k || !(0 > A || 0 > B || A > this.canvas.width || B > this.canvas.height)) return new Particle(q, s, 1 + 40 * Math.random());
    }
  }
  ...
  for (var f, d = .01 * this.speedScale / c.scale, e = 0; e < this.particles.length; e++) {
    if (f = this.particles[e], 0 < f.age && this.field.inBounds(f.x, f.y)) {
      var k = this.field.getValue(f.x, f.y);f.x += d * k.x, f.y += d * k.y, f.x < this.x0 && (f.x = this.x1 - (this.x0 - f.x)), f.x > this.x1 && (f.x = this.x0 + (f.x - this.x1)), f.age--;
  ...
  var d = this.canvas.getContext('2d'),
      ...
      l = c.dy,
      o = c.scale,
      q = d.globalCompositeOperation;d.globalCompositeOperation = 'destination-in', d.fillStyle = this.backgroundAlpha, d.fillRect(k, l, e * o, f * o), d.globalCompositeOperation = q;var s = new Vector(0, 0);d.lineWidth = this.lineWidth, d.strokeStyle = this.color;for (var u, t = 0; t < this.particles.length; t++) {
